...
exports.update = (req, res) => {
  ...
  // Define the fields you want to allow for update
  const allowedFields = ["username", "email", "password"];

  // Create an object to store the fields to be updated
  const updateFields = {};

  // Iterate through allowed fields and add them to the update object if present in the request body
  ...
  Admin.findByIdAndUpdate(req.params.adminId, updateFields, { new: true })
    .then((admin) => {
      if (!admin) {
        return res.status(404).send({
          message: "admin not found with id " + req.params.adminId,
